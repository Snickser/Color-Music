...
float cLim = 768;    // color for 1/random()
...
#ifdef PROD
byte pxRatio = 2;     // 1 for 30Led/m, 2 for 60Led/m, 4 for 100L/m, 8 for 144L/m
...
void ModeSet(byte m) {
  ...
  switch (m) {
    case 15:
      Serial.println("Mode 15: pixels");
      PCL = 34; // level, not pixels
      pfMin = 0.05;
      break;
    case 14:
      ...
      break;
    case 13:
      Serial.println("Mode 13: quad blend");
      if (Nq == 1) Nq = 2 ;
      PCL /= Nq;
      if (Nq > 3) {
        Nc = 0;
    ...
    case 10:
      ...
      break;
    case 9:
      Serial.println("Mode 9: linear + rbw");
      PCL = PixelCount;
      PCD = PCL * 0.5;
      break;
    case 8:
...
void setup() {
  ...
  randomSeed(analogRead(analogInPin)*analogRead(analogInPin));
...
//  digitalWrite(AMP, 1);
  ...
  /*
    ...
    //      HsbColor c = HsbColor(cc / cLim, 1, Br );
...
void loop() {
  ...
  if (Mode == 4) {
    ...
    if (Mode != 11 && Mode != 7 && Mode !=15) blank(0);
...
void encoder() {
  if (enc.isRight()) {
    switch (bMode) {
      case 5: // усилитель микрофона
        digitalWrite(AMP, 1);
        break;
      case 4: // 1 for 30Led/m, 2 for 60Led/m, 4 for 100L/m, 8 for 144L/m
        pxRatio++;
        meter(pxRatio, 4);
        break;
      case 3: // скорость увеличения низа
        pfMax += 0.02;
        break;
      case 2: // время обрабоки сигнала
        cs++;
        blank(0);
        meter(cs, 2);
        break;
...
void m4() {
  ...
  for (int i = 0; i < FHT_N / 2; i++) {
    ...
    if (fht_lin_out[i] > LOWP[i]) {
      ...
      target = HsbColor( (i - 3) * (cLim / (FHT_N / 2)) / cLim, 1.0, L);
...
void m5(int n, byte m) {
  ...
  for (int i = PCL - PCV; i < k - 1 ; i++) {
    ...
    if (!m) strip.SetPixelColor(PCL * 2 - i - 1, target);
  }
  //  }
...
void m1(int n) {
  //  n=PCL;
  int m = PCL / 5;
  float c, color[] = {0.56, 0.335, 0.13, 0.025, 0.9985};
...
void m0(int n) {
  ...
  int p = round(m / 2.);
  int k = n + v;
  float color[] = {0.335, 0.13, 0.027, 0.998, 0};
  float c = 0.56;
...
void m13(int n) {
    ...
    for(byte i=0;i<PixelCount;i++){
      if(random(PCL-n*0.9)==0   and n>=nLast-1){
        float RB = random(cLim+1) / cLim;
        color = HsbColor(RB, 1, Br);
      } else {
